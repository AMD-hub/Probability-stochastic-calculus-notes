...
Now let's state some basic stability properties of stopping times and the associated $\sigma$-fields.
\proppr{stability_stopping_times}{
Let $S$, $T$ and $(S_n)$ be stopping times.
\begin{enumerate}
    \item \textbf{Finite operations.}
    \begin{itemize}
        \item If $S\le T$, then
        \[ \mathcal F_S\subset\mathcal F_T. \]
        \item The random times
        \[ S\wedge T \quad\text{and}\quad S\vee T \]
        are stopping times. Moreover,
        \[ \mathcal F_{S\wedge T} = \mathcal F_S\cap\mathcal F_T. \]
    \end{itemize}

    \item \textbf{Monotone limits.}
    \begin{itemize}
        \item If $S_n\uparrow S$, then $S$ is a stopping time.
        \item If $S_n\downarrow S$ and the filtration is right-continuous, then $S$ is a stopping time and
        \[ \mathcal F_S = \bigcap_{n\ge1}\mathcal F_{S_n}. \]
    \end{itemize}

    \item \textbf{Measurable enlargements.}
    If $T$ is a stopping time and $S$ is an $\mathcal F_T$-measurable random variable satisfying $S\ge T$, then $S$ is also a stopping time.
\end{enumerate}
}{
\begin{enumerate}
    \item \textbf{Finite operations.}
    \begin{itemize}
        \item If $S\le T$, then for every $t\ge0$,
        \[ A\cap\{S\le t\} = A\cap\{T\le t\}\cap\{S\le t\}, \]
        for every $A\in\F_S$, which implies
        $\F_S\subset\F_T$.

        \item The identities
        \[ \{S\wedge T\le t\} = \{S\le t\}\cup\{T\le t\}, \]
        and
        \[ \{S\vee T\le t\} = \{S\le t\}\cap\{T\le t\}, \]
        show that $S\wedge T$ and $S\vee T$ are stopping times.

        Finally,
        \[ A\in\F_{S\wedge T} \iff A\cap\{S\wedge T\le t\}\in\F_t, \]
        together with
        \[ \{S\wedge T\le t\} = \{S\le t\}\cup\{T\le t\}, \]
        yields
        \[ \F_{S\wedge T} = \F_S\cap\F_T. \]
    \end{itemize}

    \item \textbf{Monotone limits.}
    \begin{itemize}
        \item If $S_n\uparrow S$, use
        \[ \{S\le t\} = \bigcap_{n\ge1}\{S_n\le t\}. \]

        \item If $S_n\downarrow S$, use right-continuity together with
        \[ \{S<t\} = \bigcup_{n\ge1}\{S_n<t\}. \]

        To prove \[ \F_S = \bigcap_n\F_{S_n}, \]
        first use $S\le S_n$ to obtain one inclusion, then combine the previous identity with the definition of $\F_S$ for the reverse inclusion.
    \end{itemize}

    \item \textbf{Measurable enlargements.}
    Since $S\ge T$,
    \[ \{S\le t\} = \{S\le t\}\cap\{T\le t\}, \]
    and $\{S\le t\}\in\F_T$ by assumption. Hence
    \[ \{S\le t\}\in\F_t, \]
    so $S$ is a stopping time. In particular, if $T$ is a continuous stopping time, \[ T_n= \frac{\lceil 2^n T \rceil}{2^n}\cdot \1_{\{T<\infty\}}+\infty \cdot \1_{\{T=\infty\}}, \quad n=0,1,2, \ldots \] defines a sequence of stopping times that decreases to $T$.
\end{enumerate}
}

A natural question concerns measurability with respect to the $\sigma$-field $\F_T$ associated with a stopping time $T$. Given a random variable $Y$, when is it $\F_T$-measurable? More importantly, under what conditions is the stopped process evaluated at $T$, namely $Y_T$, $\F_T$-measurable? The following lemma and theorem provide a complete characterization.
...
\thmpr{Stopped process Measurability}{stopped_process_measurability}{
    ...
    \end{aligned}
    \]
    which are both measurable (the first one since $T \wedge t$ is $\F_t$-measurable, by Proposition \ref{prop:stability_stopping_times}, and the second one by the definition of a progressive process). It follows that the restriction to $\{T \le t\}$ of the function $\omega \mapsto X_T(\omega)$ is $\F_t$-measurable, which gives the desired result by the lemma \ref{lem:measurability_wrt_stopped_filtration}. 
}
If $T$ is a stopping time, then, for every $n \in \NN$ (resp. $t \geq 0$), $n \wedge T$ (resp. $t \wedge T$) is also a stopping time (Proposition \ref{prop:stability_stopping_times}) and it follows from the preceding theorem that $Y_{n \wedge T}$ (resp. $Y_{t \wedge T}$) is $\F_{n \wedge T}$-measurable (resp. $\F_{t \wedge T}$-measurable), hence also $\F_n$-measurable (resp. $\F_t$-measurable) by Proposition \ref{prop:stability_stopping_times}.


Finally lets define hitting time. 
...
If $(X_n)_{n \in \NN}$ (resp. $(X_t)_{t \ge 0}$) is a submartingale, then
$(-X_n)_{n \in \NN}$ (resp. $(-X_t)_{t \ge 0}$) is a supermartingale. Consequently,
many results stated for supermartingales have immediate counterparts for
submartingales obtained by considering the opposite process.

A fundamental way of constructing new submartingales is through convex
transformations. Let
$\varphi:\RR\rightarrow\RR_{+}$ be a convex function, and let
$(X_n)_{n\in\NN}$ (resp. $(X_t)_{t\ge0}$) be an adapted process satisfying
$\EE[\varphi(X_n)]<\infty$ for every $n\in\NN$
(resp. $\EE[\varphi(X_t)]<\infty$ for every $t\ge0$).

If $(X_n)$ (resp. $(X_t)$) is a martingale, then
$(\varphi(X_n))$ (resp. $(\varphi(X_t))$) is a submartingale.
More generally, if $(X_n)$ (resp. $(X_t)$) is a submartingale and
$\varphi$ is increasing, then
$(\varphi(X_n))$ (resp. $(\varphi(X_t))$) is again a submartingale.

Several useful examples follow immediately from this result. If $(X_n)$
(resp. $(X_t)$) is a martingale, then $(|X_n|)$
(resp. $(|X_t|)$) is a submartingale. Likewise,
$(X_n^2)$ (resp. $(X_t^2)$) is a submartingale whenever
$\EE[X_n^2]<\infty$ (resp. $\EE[X_t^2]<\infty$). If $(X_n)$
(resp. $(X_t)$) is itself a submartingale, then its positive part
$(X_n)^+$ (resp. $(X_t)^+$) is also a submartingale.

Another basic property is that submartingales and supermartingales are
integrable on every finite time interval. More precisely, if
$(X_t)_{t\ge0}$ is either a submartingale or a supermartingale, then for
every $t>0$,

\begin{equation}
\sup_{0\le s\le t}\EE[|X_s|]<\infty.
\label{eq:martingale_local_L1_bound}
\end{equation}

Square-integrable martingales satisfy an important identity relating the
second moment of the increments to the increase of the square process.

In discrete time, let $(M_n)_{n\in\NN}$ be a square-integrable martingale
and let $n\ge m\ge1$. Then

\begin{equation}
\EE\left[
\sum_{k=m}^{n-1}(M_{k+1}-M_k)^2
\,\middle|\,\mathcal F_m
\right]
=
\EE[M_n^2-M_m^2\mid\mathcal F_m]
=
\EE[(M_n-M_m)^2\mid\mathcal F_m].
\label{eq:martingale_increment_identity_discrete}
\end{equation}

Taking expectations in \eqref{eq:martingale_increment_identity_discrete}
gives
...
=
\EE[(M_n-M_m)^2].
\label{eq:martingale_increment_identity_discrete_expectation}
\end{equation}

The continuous-time analogue is obtained by considering any subdivision
$s=t_0<t_1<\cdots<t_p=t$ of the interval $[s,t]$. If
$(M_t)_{t\ge0}$ is a square-integrable martingale, then

\begin{equation}
\EE\left[
\sum_{i=1}^{p}(M_{t_i}-M_{t_{i-1}})^2
\,\middle|\,\mathcal F_s
\right]
=
\EE[M_t^2-M_s^2\mid\mathcal F_s]
=
\EE[(M_t-M_s)^2\mid\mathcal F_s].
\label{eq:martingale_increment_identity_continuous}
\end{equation}
...
=
\EE[(M_t-M_s)^2].
\label{eq:martingale_increment_identity_continuous_expectation}
\end{equation}
...
Before studying the asymptotic behavior of martingales, we first establish several maximal inequalities. These estimates provide quantitative control over the trajectories of stochastic processes and will serve as the fundamental tools for proving the convergence theorems developed later in this chapter.
...
While maximal inequalities control the magnitude of a process, they do not by themselves describe its long-term oscillatory behavior. To study convergence, we therefore introduce our second ingredient in convergence theory of martingales : \textit{upcrossing numbers}, which quantify how often a process oscillates between two levels. The fundamental connection between bounded oscillations and convergence is provided by Doob's upcrossing inequality.
...
\subsection{Convergence consequences}
The upcrossing inequalities established in the previous section imply that the oscillations of a supermartingale are sufficiently controlled to force convergence, provided an appropriate integrability condition is imposed. More precisely, an $L^1$-bounded supermartingale converges almost surely.

\thmpr{Doob's martingale convergence theorem}{doob_martingale_convergence}{
...
Moreover, a stronger $L^p$ bounds $(p>1)$ yield convergence both almost surely and in $L^p$. 
\thmpr{Doob's Lp convergence theorem}{Doob_Lp_convergence}{
    ...
    Since the martingale $(X_n)$ is bounded in $L^p$, it is also bounded in $L^1$, and we know from Theorem \ref{thm:doob_martingale_convergence} that $X_n$ converges a.s. to a limiting random variable $X_\infty$. Moreover, by setting $X_n^* = \sup_{1\leq k \leq n} |X_k|$, the theorem \ref{thm:doob_supermartingale_inequalities} shows that, for every $n \in \ZZ_+$,
...
Finally, we state the following regularization theorem, a fundamental consequence of the upcrossing inequality and the almost sure convergence theorem for supermartingales. It states that every supermartingale with right-continuous expectation admits a càdlàg modification. Its proof can be found in \parencite[thm.~3.18]{Stochastic}.

\thmr{Doob's regularization theorem}{Doob_regularization}{
...
Every integrable random variable $Y$ induces a martingale through its conditional expectations $(\EE[Y\mid\mathcal{F}_t])$. The converse problem is therefore to characterize those martingales that admit such a representation. This is precisely the notion of a \textbf{closed martingale}. The main result of this paragraph shows that closed martingales are exactly the uniformly integrable martingales. To this end, we first introduce the notions of closed martingales and uniform integrability, then recall Vitali's convergence theorem, which provides a key tool to prove the characterization theorem which we state at the end of this section.
...
We can characterize the uniform integrability of a collection $(X_i)_{i \in I}$ in terms of the behavior of the random variables $X_i$ on events of small probability. This characterization is stated in the following proposition :
...
We now recall \textit{Vitali's convergence theorem}, which establishes the fundamental link between uniform integrability and $L^1$-convergence. 
...
Finally we state the charachterization of closed martingales in terms of uniform integrability and convergence, which is a fundamental result in the theory of martingales.
\thmpr{Doob's Uniform Integrability Convergence Theorem}{Doob_UI_convergence}{
Let $X$ be a martingale (with right-continuous sample paths if time is continuous). Then the following are equivalent:
\begin{enumerate}
    \item $X$ is closed;
    \item the family $(X_n)_{n\in\NN}$ (resp. $(X_t)_{t\ge0}$) is uniformly integrable;
    \item $X_n$ (resp. $X_t$) converges almost surely and in $L^1$ as $n\to\infty$ (resp. $t\to\infty$).
\end{enumerate}
Moreover, if these properties hold, then
$$
X_n=\EE[X_\infty\mid\F_n]
\qquad
(\text{resp. } X_t=\EE[X_\infty\mid\F_t]),
$$
where $X_\infty\in L^1$ denotes the almost sure limit.
}{
\begin{itemize}
    \item (1)$\Rightarrow$(2) If $X$ is closed, then there exists $Z\in L^1$ such that
    $X_n=\EE[Z\mid\F_n]$ (resp. $X_t=\EE[Z\mid\F_t]$). Since the family of conditional expectations
    $\{\EE[Z\mid\G]:\G\subseteq\F_\infty\}$ is uniformly integrable, $(X_n)$ (resp. $(X_t)$) is uniformly integrable.

    \item (2)$\Rightarrow$(3) Uniform integrability implies $L^1$-boundedness. Hence, by Theorem \ref{thm:doob_martingale_convergence}, the martingale converges almost surely to some integrable random variable $X_\infty$. Since $(X_n)$ (resp. $(X_t)$) is uniformly integrable and converges almost surely to $X_\infty$, Vitali's convergence theorem (Theorem \ref{thm:Vitali_convergence_theorem}) yields
    $$
    X_n\to X_\infty
    \quad
    (\text{resp. } X_t\to X_\infty)
    \qquad\text{in }L^1.
    $$

    \item (3)$\Rightarrow$(1) For every $m\in\NN$ (resp. $s\ge0$),
    $$
    X_m=\EE[X_n\mid\F_m]
    \qquad
    (\text{resp. } X_s=\EE[X_t\mid\F_s]).
    $$
    Passing to the limit as $n\to\infty$ (resp. $t\to\infty$) and using the $L^1$-continuity of conditional expectation (Theorem \ref{prop:conditional_expectation_limits_properties}) gives
    $$
    X_m=\EE[X_\infty\mid\F_m]
    \qquad
    (\text{resp. } X_s=\EE[X_\infty\mid\F_s]),
    $$
    proving that $X$ is closed.
\end{itemize}
}
\thmpr{Levy's upward theorem}{upward_theorem}{
    Let $Z\in L^1(\Omega,\A,\PP)$. Then
    $$
    \EE[Z\mid\F_n]\conv{n}\EE[Z\mid\F_\infty]
    $$
    almost surely and in $L^1$.
}{
Set $ M_n=\EE[Z\mid\F_n]$. Then $(M_n)$ is a uniformly integrable martingale. Hence, by Theorem \ref{thm:Doob_UI_convergence},
$$ M_n\longrightarrow M_\infty \qquad\text{a.s. and in }L^1. $$
Since each $M_n$ is $\F_n$-measurable and $\F_n\subseteq\F_\infty$, the limit $M_\infty$ is $\F_\infty$-measurable.
It remains to prove that
$$ M_\infty=\EE[Z\mid\F_\infty]. $$
We apply the Monotone Class Theorem.
\begin{enumerate}
\item Let $\C=\bigcup_{n\ge0}\F_n$. Since $(\F_n)$ is increasing, $\C$ is an algebra and $\sigma(\C)=\F_\infty$. 

\item Define
$$ \G=\left\{A\in\F_\infty:\int_A M_\infty\,d\PP=\int_A Z\,d\PP\right\}. $$
If $A\in\F_n$, then
$$ \int_A M_\infty\,d\PP =\lim_{k\to\infty}\int_A M_k\,d\PP =\int_A Z\,d\PP, $$
since $A\in\F_k$ for every $k\ge n$. Hence
$\C\subseteq\G.$

\item We show that $\G$ is a monotone class. If $A_m\uparrow A$ with $A_m\in\G$, then by the Monotone Convergence Theorem, 
$$ \int_A M_\infty\,d\PP = \lim_{m\to\infty} \int_{A_m}M_\infty\,d\PP = \lim_{m\to\infty} \int_{A_m}X\,d\PP = \int_AX\,d\PP, $$ 
so $A\in\G$. Similarly, if $A_m\downarrow A$ with $A_m\in\G$, then Dominated Convergence (or continuity of measure applied to the integrable functions $M_\infty$ and $X$) yields $$ \int_A M_\infty\,d\PP = \int_A X\,d\PP, $$ so $A\in\G$. Thus $\G$ is a monotone class.

\item Since $\C$ is an algebra generating $\F_\infty$ and
$\C\subseteq\G$, the Monotone Class Theorem yields
$$
\G=\F_\infty.
$$
\end{enumerate}
Therefore,
$$ \int_A M_\infty\,d\PP=\int_A Z\,d\PP, \qquad A\in\F_\infty, $$
which proves that
$$ M_\infty=\EE[Z\mid\F_\infty]. $$
}
We also need the corresponding result for decreasing families of $\sigma$-algebras.
If we have a filtration of the form $(\mathcal{F}_n:n\in-\mathbb{N})$, i.e. a collection of
$\sigma$-algebras such that
\[
\mathcal{F}_{-(n+1)}\subseteq\mathcal{F}_{-n}
\]
for all $n$, then we define
\[
\mathcal{F}_{-\infty}
=
\bigcap_n \mathcal{F}_{-n}.
\]

\thmpr{Lévy--Doob downward theorem}{downward_theorem}{
Let $(\mathcal{F}_n:n\in-\mathbb{N})$ be a collection of $\sigma$-algebras such that $ \mathcal{F}_{-(n+1)}\subseteq\mathcal{F}_{-n} $ for every $n\in\mathbb{N}$ and let
$M=(M_n:n\in-\mathbb{N})$ be a supermartingale, i.e.
\[ \mathbb{E}(M_n\mid\mathcal{F}_m)\le M_m \qquad\text{a.s.} \quad \text{for all } m\le n\le -1\]


Then if
\[ \sup \mathbb{E}M_n<\infty, \]
the process $M$ is uniformly integrable, the limit
\[ M_{-\infty} = \lim_{n\to-\infty}M_n \]
exists a.s. and in $L^1$, and
\[ \mathbb{E}(M_n\mid\mathcal{F}_{-\infty}) \le M_{-\infty} \qquad\text{a.s.}, \]
with equality if $M$ is a martingale.
}{
For every $n\in\mathbb{N}$ the upcrossings inequality applies to the
supermartingale $ (M_k:k=-n,\ldots,-1). $ So by reasoning as in the proof of Theorem~\ref{thm:doob_martingale_convergence} we see that the limit
\[ M_{-\infty} = \lim_{n\to-\infty}M_n \]
exists and is finite almost surely.

Now for all $K>0$ and $n\in-\mathbb{N}$ we have
\[ \int_{|M_n|>K}|M_n|\,d\mathbb{P} = -\int_{M_n<-K}M_n\,d\mathbb{P} - \int_{M_n>K}M_n\,d\mathbb{P} + \mathbb{E}M_n. \]

As $n$ tends to $-\infty$, the expectation $\mathbb{E}M_n$ increases to a finite limit. So for an arbitrary $\varepsilon>0$, there exists an $m\in-\mathbb{N}$ such that
\[ \mathbb{E}M_n-\mathbb{E}M_m\le\varepsilon \]
for all $n\le m$. 
Together with the supermartingale property this implies that for all
$n\le m$,
\begin{align*}
\int_{|M_n|>K}|M_n|\,d\mathbb{P}
&\le \int_{M_n<-K}M_m\,d\mathbb{P} - \int_{M_n>K}M_m\,d\mathbb{P} + \mathbb{E}M_m+\varepsilon \\
&= \int_{|M_m|>K}|M_m|\,d\mathbb{P} +\varepsilon.
\end{align*}

Hence, to prove the uniform integrability of $M$ it suffices (in view of
Proposition~\ref{prop:characterstic_UI}) to show that by choosing $K$ large enough, we can make
\[ \mathbb{P}(M_n>K) \]
arbitrarily small for all $n$ simultaneously.

Now consider the process $M^-=\max\{-M,0\}.$  It is an increasing, convex function of the submartingale $-M$, whence
it is a submartingale itself. In particular,
\[ \mathbb{E}M_n^-\le \mathbb{E}M_{-1}^- \quad \text{for all } n\in-\mathbb{N}, \]

It follows that
\[ \mathbb{E}|M_n| = \mathbb{E}M_n + 2\mathbb{E}M_n^- \le \sup\mathbb{E}M_n +2\mathbb{E}|M_{-1}|, \]

and, consequently,
\[ \mathbb{P}(|M_n|>K) \le \frac{1}{K} \left( \sup\mathbb{E}M_n + 2\mathbb{E}|M_{-1}| \right). \]

So indeed, $M$ is uniformly integrable.
The limit $M_{-\infty}$ therefore exists in $L^1$ as well and the proof
can be completed by reasoning as in the proof of the upward theorem.
}




% \textbf{Backward supermartingales}
...
\thmpr{Optional stopping theorem -- martingale}{optional_stopping_martingale}{
    ...
    Finally, if $S$ and $T$ are two stopping times such that $S \le T$, the property $\F_S \subset \F_T$ ((1) in Proposition \ref{prop:stability_stopping_times}) and Theorem of nested \sfields in conditional expectation \ref{thm:conditional_nested_sigma_fields} imply that
    ...
    By Proposition \ref{prop:stability_stopping_times}, $(T_n)$ and $(S_n)$ are two sequences of stopping times that decrease respectively to $T$ and to $S$. Moreover, we have $S_n \leq T_n$ for every $n \geq 0$.
...
Let $(Z_n)_{n\in\NN}$ (resp. $(Z_t)_{t \geq 0}$) be a nonnegative supermartingale (with right-continuous sample paths if time is continnuous). Let $U$ and $V$ be two stopping times such that $U \le V$. Then $Z_U$ and $Z_V$ are in $L^1$, and
    ...
    Let's begin with the discrete time case. We first note that $(X_n)_{n \in \NN}$ is bounded in $L^1$. Theorem \ref{thm:doob_martingale_convergence} then shows that $X_n$ converges a.s. to $X_\infty$, so that the definition of $X_T$ makes sense for any stopping time, even on the event $\{T = \infty\}$. 
    ...
    Let us come back to the general case where $S$ and $T$ satisfy $S \leq T$ but are no longer assumed to be bounded. Let $A \in \F_S$, and $k \in \NN$. Then $A \cap \{S \leq k\}$ belongs both to $\F_k$ (by the definition of $\F_S$) and to $\F_S$ (recall that $S$ is $\F_S$-measurable). From Proposition \ref{prop:stability_stopping_times}, $A \cap \{S \leq k\}$ belongs to $\F_{S \wedge k}$. Hence, by applying the bounded case to the stopping times $S \wedge k$ and $T \wedge k$, we get
    ...
    \[
    U_n = 2^{-n}\left\lceil 2^n U \right\rceil,
    \qquad
    V_n = 2^{-n}\left\lceil 2^n V \right\rceil.
    \]
    in such a way (by Proposition \ref{prop:stability_stopping_times}) that $(U_n)$ and $(V_n)$ are two sequences of bounded stopping times that decrease respectively to $U$ and $V$, and additionally we have $U_n \le V_n$ for every $n \ge 0$. The right-continuity of sample paths ensures that $Z_{U_n} \to Z_U$ and $Z_{V_n} \to Z_V$ a.s. as $n \to \infty$. 
    Then, by the optional stopping theorem for discrete supermartingales in the case of bounded stopping times (which we just proved) applied to $U_{n+1}\leq U_n$, with respect to the filtration $(\F_{k/2^{n+1}})_{k\ge0}$, we have for every $n \ge 0$,
    \[ Z_{U_{n+1}} \ge \EE[Z_{U_n} \mid \F_{U_{n+1}}]. \]
    Setting $Y_n = Z_{U_{-n}}$ and $\H_n = \F_{U_{-n}}$, for every integer $n \le 0$, we get that the sequence $(Y_n)_{n\le0}$ is a backward supermartingale with respect to the filtration $(\H_n)_{n\le0}$. Since, for every $n \ge 0$, $\EE[Z_{U_n}] \le \EE[Z_0]$ (by another application of the discrete optional stopping theorem), the sequence $(Y_n)_{n\le0}$ is bounded in $L^1$, and by the Doob-Levy downward theorem for backward supermartingales in Theorem \ref{thm:downward_theorem}, it converges in $L^1$. Hence the convergence of $Z_{U_n}$ to $Z_U$ also holds in $L^1$ and similarly the convergence of $Z_{V_n}$ to $Z_V$ holds in $L^1$. Since $U_n \le V_n$, by yet another application of the discrete optional stopping theorem, we have $\EE[Z_{U_n}] \ge \EE[Z_{V_n}]$. Using the $L^1$-convergence of $Z_{U_n}$ and $Z_{V_n}$ we can pass to the limit $n \to \infty$ and obtain that $\EE[Z_U] \ge \EE[Z_V]$ as claimed.
...
The decomposition of as a difference of two ﬁnite positive measures on $[0, T]$
...
function with respect to a function with ﬁnite variation
...
By Proposition \ref{prop:other_properties_local_martingale}, $N$ is a (bounded) martingale. Let $t > 0$ and let $0 = t_0 < t_1 < \dots < t_p = t$ be any subdivision of $[0, t]$. Then, from increment formula established at  \ref{eq:martingale_increment_identity_continuous_expectation}, we have
\[
\begin{aligned}
