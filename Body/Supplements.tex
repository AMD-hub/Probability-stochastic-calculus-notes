...
\thmr{Bounded Linear Operators are Continuous}{bounded_linear_operators}{
Let $X$ and $Y$ be normed vector spaces, and let $T:X\to Y$ be a linear mapping.
...
\begin{enumerate}
    ...
    \[
    \|T x\|_Y \leq C\|x\|_X, \qquad x\in X.
    \]
\end{enumerate}
In particular, every bounded linear operator is continuous.
}

\thmr{Extension of an Isometry}{extension_isometry}{
Let $X$ be a normed vector space, let $Y$ be a Banach space, and let $E\subset X$ be a dense linear subspace.
Suppose that $T:E\to Y$ is a linear isometry, namely
\[
\|T x\|_Y = \|x\|_X, \qquad x\in E.
\]
Then there exists a unique linear isometry
\[
\widetilde T:X\to Y
...
\defnr{Brownnian Motion}{brownian_motion}{
...
\defnr{$(\mathcal{F}_t)$-Brownian Motion}{F_t_brownian_motion}{
A real-valued process $B=(B_t)_{t\ge0}$ is called an
\emph{$(\mathcal{F}_t)$-Brownian motion} if $B$ is a Brownian motion,
$B$ is adapted to the filtration $(\mathcal{F}_t)_{t\ge0}$, and
$B$ has independent increments with respect to $(\mathcal{F}_t)$.

Similarly, an $\mathbb{R}^d$-valued process
$B=(B_t)_{t\ge0}$ is called a
\emph{$d$-dimensional $(\mathcal{F}_t)$-Brownian motion}
if $B$ is a $d$-dimensional Brownian motion, $B$ is adapted to
$(\mathcal{F}_t)_{t\ge0}$, and has independent increments with
respect to $(\mathcal{F}_t)$.

If $B$ is a (possibly $d$-dimensional) Brownian motion and
$(\mathcal{F}_t^B)_{t\ge0}$ denotes its (possibly completed)
canonical filtration, then $B$ is an
$(\mathcal{F}_t^B)$-Brownian motion.
}
...
The part (1) is in fact defining and calculating the \emph{quadratic variation} of Brownian motion, this concept is studied in more details in Section \ref{sec:quadratic-variation} and use the notation $\langle B, B \rangle = t$.
...
\]
Ând we have the following result.
\propr{brownian_motion_independence}{
Let $B=(B_t)_{t\ge0}$ and $B'=(B'_t)_{t\ge0}$ be two $(\mathcal{F}_t)$-Brownian motions. Assume that $B$ and $B'$ are \emph{independent}, that is,
\[
\sigma(B_t:t\ge0)
\quad\text{and}\quad
\sigma(B'_t:t\ge0)
...
\section{Levy's Characterisation}
\thmr{Levy's Characterisation}{levy_characterisation}{
Let $X=(X^1,\ldots,X^d)$ be an adapted process with continuous sample paths. The following are equivalent:
\begin{enumerate}
    \item $X$ is a $d$-dimensional $(\mathcal{F}_t)$-Brownian motion.
    \item The processes $X^1,\ldots,X^d$ are continuous local martingales, and
    \[
    \langle X^i,X^j\rangle_t=\delta_{ij}t,
    \qquad \forall\, t\ge0,\; i,j\in\{1,\ldots,d\},
    \]
